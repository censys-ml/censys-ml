...
assign_value 'Is there an existing source Config y/n' ''
...
if [ $EXISTING_CONFIG == y ] || [ $EXISTING_CONFIG == Y ]; then
    ...
    if [ $SOURCE_TYPE == file ]; then
        ...
        export IN_FILE=$VECTOR_DIR"/input/"$func_result
    ...
    export SOURCE_TYPE
    export CONFIG_PATH=$VECTOR_DIR"/source_configs/"$SOURCE_TYPE".toml"
fi
...
# debug
vector --config $CONFIG_PATH $VECTOR_DIR/vector.toml --verbose --threads 4
